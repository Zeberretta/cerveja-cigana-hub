...
-- 1. Fix marketplace_items view - remove SECURITY DEFINER and add proper RLS
...
-- Enable RLS on marketplace_items view
ALTER VIEW public.marketplace_items SET (security_barrier = true);

-- Create RLS policies for marketplace_items view
...
-- 3. Add RLS policies to restrict admin data access
CREATE POLICY "Admins can view all profiles" 
ON public.profiles 
FOR SELECT 
USING (public.is_admin() OR auth.uid() = user_id);

CREATE POLICY "Admins can view all registrations - cigano" 
ON public.cigano_registrations 
FOR SELECT 
USING (public.is_admin() OR auth.uid() = user_id);

CREATE POLICY "Admins can view all registrations - fabrica" 
ON public.fabrica_registrations 
FOR SELECT 
USING (public.is_admin() OR auth.uid() = user_id);

CREATE POLICY "Admins can view all registrations - fornecedor" 
ON public.fornecedor_registrations 
FOR SELECT 
USING (public.is_admin() OR auth.uid() = user_id);

CREATE POLICY "Admins can view all registrations - bar" 
